...
// Get the total count of rows
 ...
 $sql = "SELECT COUNT(ticket) AS total FROM tickets WHERE client_id=$id && job_card='$x' && dates>= '$date' && dates<= '$date2'";
...
$sql="SELECT * FROM tickets  WHERE client_id=$id && job_card='$x' && dates>= '$date' && dates<= '$date2' ORDER BY ticket ASC $limit";
...
$list = '';
?>
   <h4><a style ="color:white">DOWNLOAD FILE TABLE.</a></h4>
...
<?php
 ...
 if ($query->num_rows > 0) {
    ...
    while($row = $query->fetch_assoc()) {
	    $ticket=$row['ticket'];
		$sql3="SELECT * FROM upload WHERE ticket='$ticket'";
	...
	?>	
		...
		<td><a href="downloadfile2.php?download_file= <?php echo $row3['filename'] ?>">Download</a> </td> 
        </tr>
        <?php
    }
